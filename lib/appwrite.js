...
export const createUserResponsavel = async (email,password,username,
    cpf,endereco, bairro, rg, whatsapp, birthDate, contatoalternativo, parentesco, role) => {
    try {
        const newAccount = await account.create(
            ID.unique(),
            email,
...
export const createUserAtleta = async (
  ...
  try {
    ...
    // Criação de nova conta no Appwrite (caso email e senha sejam fornecidos)
    if (email && password) {
      newAccount = await account.create(ID.unique(), email, password);
    ...
    // Dados do atleta que serão armazenados no documento
    const documentData = {
      email: email || null,
      username,
      ...
      ano,
      whatsapp,
      createdByUsername,   // Quem criou o registro
      createdByUserId,     // ID de quem criou o registro
      role: 'atleta'       // Função do usuário como 'atleta'
    };

    // Caso uma nova conta tenha sido criada, atribui o ID do usuário ao documento
    if (newAccount) {
      documentData.userId = newAccount.$id;
    }

    // Criação do documento na coleção de alunos
