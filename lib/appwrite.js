...
export const salvarImagemTreino = async (file, title, userId) => {
  ...
  try {
    ...
    // Criação do documento na coleção do Appwrite após o upload bem-sucedido
    const response = await databases.createDocument(
      appwriteConfig.databaseId,
      appwriteConfig.galeriaCollectionId,
      ID.unique(),
  ...
  try {
    ...
    for (const imagem of imagens) {
      ...
      try {
        ...
        const { mimeType, uri } = imagem;
        const asset = { uri, name: imageName, type: mimeType || 'image/jpeg' };
   
        const fileUploaded = await storage.createFile(
          appwriteConfig.bucketRelatoriosID,
          ID.unique(),
          asset
        );
  ...
  export const getUsersById = async (userId) => {
    try {
      const user = await databases.getDocument(
        appwriteConfig.databaseId,  // ID do banco de dados
        appwriteConfig.users_v1CollectionId,  // ID da coleção de alunos
