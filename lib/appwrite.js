...
export const appwriteConfig = {
    ...
    faturas_v1CollectionId: '67cdd3f1002155ac95d4',
...
export const createUserResponsavel = async (email,
  ...
  export const createUserAtleta = async (
    ...
    try {
      ...
      const existingAtleta = await databases.listDocuments(
        appwriteConfig.databaseId,
        appwriteConfig.atletas_v1CollectionId,
        [Query.equal('cpf', cpf)]
      );
  
      if (existingAtleta.total > 0) {
        throw new Error('Atleta já cadastrado com este CPF.');
      }
...
export const updateTurma = async (turmaId, updatedData) => {
  try {
    const response = await databases.updateDocument(
      appwriteConfig.databaseId,            // ID do banco de dados
      appwriteConfig.turmaCollectionId,     // ID da coleção de turmas
      turmaId,                              // ID da turma que será atualizada
      {
        title: updatedData.title,
        Qtd_Semana: updatedData.Qtd_Semana,
        Dia1: updatedData.Dia1,
        Dia2: updatedData.Dia2,
        Dia3: updatedData.Dia3,
        Local: updatedData.Local,
        MaxAlunos: updatedData.MaxAlunos,
        Sub: updatedData.Sub,
        Horario_de_inicio: updatedData.Horario_de_inicio,
        Horario_de_termino: updatedData.Horario_de_termino
      }
    );

    return response; // Retorna a resposta da API para confirmar a atualização
  } catch (error) {
    throw new Error("Erro ao atualizar turma: " + error.message);
  }
};

export const createTurma = async (title, Qtd_Semana, Dia1, Dia2, Dia3, Local, MaxAlunos, Sub, Horario_de_inicio, Horario_de_termino) => {
    try {
      const response = await databases.createDocument(
        appwriteConfig.databaseId, 
        appwriteConfig.turmaCollectionId, 
  ...
  export const savePlano = async (planoData) => {
    try {
      const response = await databases.createDocument(
        appwriteConfig.databaseId,
        appwriteConfig.faturas_v1CollectionId,
        ID.unique(),
        planoData
      );
      return response;
    } catch (error) {
      throw new Error(`Erro ao salvar o plano: ${error.message}`);
    }
  };
...
export const updateUserData = async (userId, updatedField, newValue, role) => {
  try {
    const collectionId =
      role === 'responsavel'
        ? '671b0484003da7c2e564'
        : role === 'profissional'
        ? '67188aa5001275afc709'
        : null;

    if (!collectionId) throw new Error('Role inválido para atualização.');

    const updateData = {};
    if (updatedField.toLowerCase() === 'cpf') {
      updateData.cpf = newValue;
    } else if (updatedField.toLowerCase() === 'whatsapp' || updatedField.toLowerCase() === 'whats') {
  ...
  export const deleteUser = async (userId) => {
    try {
      // Deletar o documento na tabela users_v1
      ...
      if (userResponse.documents.length === 0) {
        throw new Error("Nenhum documento encontrado para o userId fornecido na tabela users_v1.");
      }
  
      const userDocumentId = userResponse.documents[0].$id;
  
      const deleteUserResponse = await databases.deleteDocument(
        appwriteConfig.databaseId,
        appwriteConfig.users_v1CollectionId,
        userDocumentId
      );
  
      // Verificar e deletar da tabela correspondente: atletas_v1, profissionais_v1 ou responsaveis_v1
      const collections = [
        appwriteConfig.atletas_v1CollectionId,
        appwriteConfig.profissionais_v1CollectionId,
        appwriteConfig.responsaveis_v1CollectionId
      ];
      ...
      for (const collectionId of collections) {
        ...
        if (response.documents.length > 0) {
          ...
          deleteResults.push({ collection: collectionId, status: "Deletado" });
        } else {
          deleteResults.push({ collection: collectionId, status: "Nenhum documento encontrado" });
        }
      }
  
      return {
        userDeleted: deleteUserResponse,
        relatedDeletions: deleteResults
      };
    } catch (error) {
  ...
  export const getAllAlunos = async () => {
    try {
        ...
        while (hasMore) {
            ...
            alunos = alunos.concat(response.documents);
            offset += limit;
            hasMore = response.documents.length === limit; // Se retornou menos que o limite, não há mais alunos para buscar
        }
...
export const getAllResponsaveis = async () => {
  try {
    ...
    while (hasMore) {
      ...
      responsaveis = responsaveis.concat(response.documents);
      offset += limit;
      hasMore = response.documents.length === limit; // Se retornou menos que o limite, não há mais responsáveis para buscar
    }
...
export const getAllFaturas = async () => {
  try {
      let faturas = [];
      let hasMore = true;
      const limit = 100; // Máximo permitido por requisição
      let lastDocument = null; // Armazena o último documento para paginação

      while (hasMore) {
          // Definir a query inicial com limite
          let queries = [Query.limit(limit)];
          
          // Se houver um último documento, usamos cursor para buscar a próxima página
          if (lastDocument) {
          ...
          // Adicionar os novos documentos ao array de faturas
          faturas = faturas.concat(response.documents);

          // Se o número de documentos retornados for menor que o limite, chegamos ao fim
          hasMore = response.documents.length === limit;

          // Atualiza o último documento para o próximo ciclo de paginação
          if (hasMore) {
      ...
      throw new Error("Erro ao buscar todas as faturas.");
  }
};
...
export const getAllProfissionais = async () => {
  try {
    ...
    while (hasMore) {
      ...
      profissionais = profissionais.concat(response.documents);
      offset += limit;
      hasMore = response.documents.length === limit; // Se retornou menos que o limite, não há mais profissionais para buscar
    }
...
export const getAllUsers = async () => {
  try {
    ...
    while (hasMore) {
      ...
      offset += limit;
      hasMore = response.documents.length === limit; // Se retornou menos que o limite, não há mais usuários para buscar
    }
...
export const getUsersById = async (userId) => {
  ...
  export const updateStatusFatura = async (docId, status_pagamento) => {
    try {
      // Buscar a fatura pelo ID do documento
      const response = await databases.listDocuments(
        appwriteConfig.databaseId,
        appwriteConfig.faturas_v1CollectionId,
      ...
      if (response.documents.length === 0) {
        throw new Error("Nenhuma fatura encontrada para o docId fornecido.");
      }

      const documentId = response.documents[0].$id;

      const updatedFatura = await databases.updateDocument(
        appwriteConfig.databaseId,
        appwriteConfig.faturas_v1CollectionId,
        documentId,
        { status_pagamento }
  ...
  export const updateStatusFaturaPix = async (docId, status_pagamento, paymentId) => {
    try {
      // Buscar a fatura pelo ID do documento
      const response = await databases.listDocuments(
        appwriteConfig.databaseId,
  ...
  export const updateStatusFaturaCartao = async (docId, status_pagamento, paymentId) => {
    try {
        // Buscar a fatura pelo ID do documento
        const response = await databases.listDocuments(
            appwriteConfig.databaseId,
        ...
        const { createdAtCustom, atletaId, responsavelId } = response.documents[0];

        if (!createdAtCustom || !atletaId || !responsavelId) {
            throw new Error("Campos obrigatórios não encontrados na fatura.");
        }

        // Buscar todas as faturas que possuem o mesmo createdAtCustom, atletaId e responsavelId
        const faturasResponse = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.faturas_v1CollectionId,
            [
                Query.equal("createdAtCustom", createdAtCustom),
                Query.equal("atletaId", atletaId),
                Query.equal("responsavelId", responsavelId)
            ]
        );

        if (faturasResponse.documents.length === 0) {
            throw new Error("Nenhuma outra fatura encontrada com os mesmos critérios.");
        }

        // Atualizar todas as faturas encontradas
        for (const fatura of faturasResponse.documents) {
            await databases.updateDocument(
                appwriteConfig.databaseId,
                appwriteConfig.faturas_v1CollectionId,
                fatura.$id,
                { status_pagamento, id_transacao: paymentId }
            );
        }

        return { message: "Faturas atualizadas com sucesso!" };
    } catch (error) {
        console.error("Erro ao atualizar as faturas:", error);
        throw new Error("Erro ao atualizar status das faturas.");
    }
};
...
export const deleteTurma = async (turmaId) => {
  try {
    ...
    throw new Error('Erro ao deletar a turma: ' + error.message);
  }
};
...
export const deleteTreino = async (id) => {
  try {
    const response = await databases.deleteDocument(
      appwriteConfig.databaseId,
      appwriteConfig.personalizados_v1CollectionId, // Identificador da coleção de treinos
      id // ID do treino que será deletado
    );
    return response;
  } catch (error) {
...
export const updateStatusPagamento2 = async (alunoId, novoStatus) => {
  try {
    const alunoResponse = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.atletas_v1CollectionId,
      [Query.equal('userId', alunoId)]
    );

    if (alunoResponse.documents.length === 0) {
      throw new Error('Aluno não encontrado.');
    }

    const alunoDoc = alunoResponse.documents[0];
    const alunoDocId = alunoDoc.$id;
    const endDate = alunoDoc.end_date;
    ...
    // 3️⃣ Buscar o histórico de pagamentos na tabela historico_payments_v1
    const historicoResponse = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.historico_payments_v1CollectionId,
      [
        Query.equal('userId', alunoId),
        Query.equal('end_date', endDate)
      ]
    );

    if (historicoResponse.documents.length === 0) {
      throw new Error('Histórico de pagamentos não encontrado.');
    }

    const historicoDocId = historicoResponse.documents[0].$id;

    // 4️⃣ Atualizar o status do pagamento na tabela historico_payments_v1
    await databases.updateDocument(
      appwriteConfig.databaseId,
      appwriteConfig.historico_payments_v1CollectionId,
      historicoDocId,
...
export const updateAlunoTurma = async (alunoId) => {
  try {
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.atletas_v1CollectionId,
    ...
    const documentId = response.documents[0].$id;  // Pega o ID do primeiro documento encontrado

    // Atualiza o campo turmaId para null
    await databases.updateDocument(
      appwriteConfig.databaseId,
      appwriteConfig.atletas_v1CollectionId,
      documentId,
